...
def main(args):
    ...
    if not args.learn:
        ...
        #####设置bbox的输入为0
        bbox_train = [torch.zeros(len(seq), 4).tolist() for seq in bbox_train]
        bbox_valid = [torch.zeros(len(seq), 4).tolist() for seq in bbox_valid]
        bbox_test = [torch.zeros(len(seq), 4).tolist() for seq in bbox_test]
        ####
        ...
        ###acc设置为0
        acc_train = torch.zeros_like(torch.tensor(acc_train), device=device)
        acc_valid = torch.zeros_like(torch.tensor(acc_valid), device=device)
        acc_test = torch.zeros_like(torch.tensor(acc_test), device=device)
        ###
    ...
    #################################### 自动保持excel文件
    # log to Excel
    excel_path = os.path.join(checkpoint_dir, 'experiments_location_Input_ablation_bnks_8.xlsx')

    record = {
        'checkpoint': os.path.splitext(os.path.basename(__file__))[0],
        'location': checkpoint_filename,
        'mode': 'train_val_test',
        'epochs': args.epochs,
        'lr': args.lr,
