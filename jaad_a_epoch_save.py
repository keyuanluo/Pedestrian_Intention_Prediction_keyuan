...
def train_with_epochly_excel(model,
    ...
    for epoch in range(1, epochs+1):
        ...
        for bbox, label, vel, traj in train_loader:
            ...
            preds, pts, s_cls, s_reg = model(bbox, vel)
...
def evaluate(model, loader, cls_crit, reg_crit):
    ...
    y_true, y_pred = [], []
    with torch.no_grad():
        for bbox, label, vel, traj in loader:
            label = label.reshape(-1,1).to(device).float()
            bbox  = bbox.to(device)
            vel   = vel.to(device)
            end_pt= traj.to(device)[:, -1, :4]

            pred, pts, s_cls, s_reg = model(bbox, vel)
            pred = pred.view(-1, 1) ######## 新增
...
def main(args):
    ...
    latest_path, best_path = train_with_epochly_excel(model, train_loader, valid_loader, test_loader, cls_criterion, reg_criterion, optimizer, checkpoint_filepath, writer, args=args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str, default='/media/robert/4TB-SSD/checkpoints/JAAD_all_model_best_01.pt',help='测试模式下指定 .pt 文件路径')
