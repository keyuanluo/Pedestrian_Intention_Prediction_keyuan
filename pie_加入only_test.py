...
from utils.pie_preprocessing_new import *

import torch
from torch import nn
from torch.utils.data import TensorDataset, DataLoader
from torch.utils.tensorboard import SummaryWriter
from model.main_model_pie import Model
...
def main(args):
    ...
    if args.test_only:
        ...
        #######################################用于记录excel文件
        # Log to Excel
        excel_path = os.path.join(checkpoint_dir, 'experiments.xlsx')
    ...
    if not args.learn:  # 如果args.learn为False，则真实训练， 读取真实数据
        ...
        tte_seq_train, traj_seq_train = tte_dataset(balanced_seq_train, data_opts['tte'], 0.8,
                                                    args.times_num)  # 生成训练集的tte和轨迹

        seq_valid = imdb.generate_data_trajectory_sequence('val', **data_opts)
        balanced_seq_valid = balance_dataset(seq_valid)
        tte_seq_valid, traj_seq_valid = tte_dataset(balanced_seq_valid, data_opts['tte'], 0.8, args.times_num)

        seq_test = imdb.generate_data_trajectory_sequence('test', **data_opts)
        tte_seq_test, traj_seq_test = tte_dataset(seq_test, data_opts['tte'], 0.8, args.times_num)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str, default='checkpoints/PIE-master_01.pt',help='测试模式下指定 .pt 文件路径')
