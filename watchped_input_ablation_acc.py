...
def main(args):
    ...
    if not args.learn:
        ...
        ########把输入的速度设置为0
        vel_train = torch.zeros_like(torch.tensor(vel_train), device=device)
        vel_valid = torch.zeros_like(torch.tensor(vel_valid), device=device)
        vel_test = torch.zeros_like(torch.tensor(vel_test), device=device)
        ########
