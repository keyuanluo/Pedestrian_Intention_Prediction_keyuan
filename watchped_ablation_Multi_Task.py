...
from utils._watchped_preprocessing_ablation_Multi_Task import *

import torch
from torch import nn
from torch.utils.data import TensorDataset, DataLoader
from torch.utils.tensorboard import SummaryWriter
from model.main_model_ablation_Multi_Task import Model
...
def main(args):
    if not args.learn:
        ...
        # 修改后：增加加速度数据
        trainset = TensorDataset(X_train, Y_train, vel_train, acc_train)
        validset = TensorDataset(X_valid, Y_valid, vel_valid, acc_valid)
        testset = TensorDataset(X_test, Y_test, vel_test, acc_test)
    ...
    train(model, train_loader, valid_loader, cls_criterion, optimizer, checkpoint_filepath, writer, args=args)
