...
def main(args):
    ...
    if not args.learn:
        ...
        ####速度设置为0
        vel_train = torch.zeros_like(torch.Tensor(vel_train))
        vel_valid = torch.zeros_like(torch.Tensor(vel_valid))
        vel_test = torch.zeros_like(torch.Tensor(vel_test))
        ###
        ...
        #####acc设置为0
        acc_train = torch.zeros_like(torch.Tensor(acc_train))
        acc_valid = torch.zeros_like(torch.Tensor(acc_valid))
        acc_test = torch.zeros_like(torch.Tensor(acc_test))
        #####
