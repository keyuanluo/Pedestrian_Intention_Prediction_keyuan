...
from model.BottleNecks_three_input_multi_attention_PIE_bbox_motion import Bottlenecks
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        ###bbox_motion + token
        self.bbox_motion_embedding = EmbedPosEnc(args.bbox_input, d_model)
        self.bbox_motion_token = nn.Parameter(torch.ones(1, 1, d_model))
        ...
        ###新加的bbox_motion
        self.bbox_motion_att = nn.ModuleList()
        self.bbox_motion_ffn = nn.ModuleList()
        ###
        ...
        for _ in range(self.num_layers):
            ...
            ###新加的bbox_motion
            self.bbox_motion_att.append(AttentionBlocks(d_model,args.num_heads))
            self.bbox_motion_ffn.append( FFN(d_model, hidden_dim) )
            ###
    ...
    def forward(self, bbox, bbox_motion,vel):
        """
        bbox: [B, T, 4]
        vel : [B, T, 2]
        """
        ...
        bbox_motion = self.bbox_motion_embedding(bbox_motion, self.bbox_motion_token)
        bbox_motion = self.dropout_att(bbox_motion)
        ...
        ######新加的bbox_motion
        bbox_motion = self.bbox_motion_att[0](bbox_motion)
        bbox_motion = self.dropout_att(bbox_motion)
        token = torch.cat([token, bbox_motion[:, 0:1, :]], dim=1)
        #####
        ...
        ###新加的bbox_motion
        bbox_motion = torch.cat([token_new, bbox_motion[:, 1:, :]], dim=1)
        ###
        ...
        ###新加的bbox_motion
        bbox_motion =self.bbox_motion_ffn[0](bbox_motion)
        bbox_motion = self.dropout_ffn(bbox_motion)
        ###
        ...
        for i in range(1, self.num_layers):
            ...
            ####新加的bbox_motion
            bbox_motion = self.bbox_motion_att[i](bbox_motion)
            bbox_motion = self.dropout_att(bbox_motion)
            token = torch.cat([token, bbox_motion[:, 0:1, :]], dim=1)
            ###
            ...
            bbox = self.dropout_ffn(bbox)
            ####新加的bbox_motion
            bbox_motion = self.bbox_ffn[i](bbox_motion)
            bbox_motion = self.dropout_ffn(bbox_motion)
            ###
        ...
        # --- 最终预测
        cls_out = torch.cat([bbox[:, 0:1, :], bbox_motion[:, 0:1, :], vel[:, 0:1, :]], dim=1)   # [B, 2, d_model]
        cls_flat = cls_out.flatten(start_dim=1)                         # [B, 2*d_model]
        end_point = self.endp(cls_flat)                                 # 端点回归

        bnk  = self.relu(self.time_transformer(self.bottlenecks(bbox, bbox_motion, vel)))    # temporal bottleneck
