...
from model.BottleNecks_three_input_multi_attention_JAAD_bbox_motion import Bottlenecks
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.bbox_motion_token = nn.Parameter(torch.ones(1, 1, d_model), requires_grad=True)
    ...
    def forward(self, bbox,bbox_motion, vel):
        ...
        bbox_motion = self.bbox_motion_ffn[1](bbox_motion)
        ...
        for i in range(self.num_layers - 1):
            ...
            bbox_motion = self.bbox_motion_att[i + 1](bbox_motion)
            ...
            ###新加的bbox_motion
            bbox_motion = self.bbox_motion_ffn[i + 1](bbox_motion)
            ###
            vel = self.vel_ffn[i + 1](vel)
            token = self.cross_ffn[i + 1](token)[:, 0:1, :]


        cls_out = torch.cat([bbox[:, 0:1, :], bbox_motion[:, 0:1, :],vel[:, 0:1, :]], dim=1) # 拼接bbox的token和vel的token
        cls_out_flatten = torch.flatten(cls_out, start_dim=1) # 展平
        end_point = self.endp(cls_out_flatten) # 全连接层预测endpoint

        bnk = self.relu(self.time_transformer(self.bottlenecks(bbox,bbox_motion,vel))) # Bottlenecks
