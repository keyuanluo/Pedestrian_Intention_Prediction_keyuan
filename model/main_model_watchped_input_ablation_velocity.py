...
class Model(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.vel_embed  = nn.Linear(args.vel_input, args.d_model)
    ...
    def forward(self, vel, lengths):
        """
        vel:     [B, T_max, vel_input]——归一化后 + 0 填充
        lengths: [B]                     ——每条序列真实长度
        """
        x = self.vel_embed(vel)                # [B,T,d_model]
        x = self.pos_enc(x)
        # padding mask
        max_len = vel.size(1)
        arange  = torch.arange(max_len, device=vel.device).unsqueeze(0)
